...
We can have problems when having variables with the same name \footnote{As you can see in the next example.}, so we need to use a technique called \textbf{standardizing apart} which changes one of the two sentence's variables to avoid name clashes.\\
...
\[\exists x\ Owns(EasterIsland,)\wedge KinderEgg(x)\]
